  ...
  if (!list) {
    return (
      <div className="px-4 py-8 text-center">
        <p className="text-[#8A7F85]">List not found.</p>
        <Link href="/lists" className="text-[#B8A9D4] text-sm mt-2 inline-block">
          Back to lists
        </Link>
      </div>
    );
    ...
    <div className="px-4 py-4">
        ...
        {editingName ? (
          <div className="flex-1 flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-1.5 bg-[#FFFBF5] border border-[#F0EBE6] rounded-xl text-sm text-[#3D3539] focus:outline-none focus:ring-2 focus:ring-[#B8A9D4]/40"
              autoFocus
              onKeyDown={(e) => e.key === "Enter" && handleRename()}
            />
            <button onClick={handleRename} className="text-xs text-[#B8A9D4] font-medium">Save</button>
            <button onClick={() => setEditingName(false)} className="text-xs text-[#8A7F85]">Cancel</button>
          </div>
        ) : (
          <div className="flex-1 flex items-center gap-2">
            ...
            {isOwner && !list.is_default && (
              <button onClick={() => setEditingName(true)} className="p-1 text-[#8A7F85] hover:text-[#3D3539]">
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <p className="text-[#8A7F85] text-sm mb-4">No books in this list yet.</p>
          {isOwner && (
            <Link
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.notes && (
                  <p className="text-xs text-[#8A7F85] mt-1 line-clamp-2 italic">{item.notes}</p>
                )}
              </div>
              {isOwner && (
                <button
      ...
      {isOwner && (
        <div className="mt-6 space-y-3">
          <Link
            href={`/lists/add?listId=${list.id}`}
            className="block w-full py-3 text-center text-sm font-medium bg-[#F8F5F0] hover:bg-[#F0EBE6] text-[#3D3539] rounded-2xl transition-all"
          >
            Add Books to List
          </Link>
          {!list.is_default && (
